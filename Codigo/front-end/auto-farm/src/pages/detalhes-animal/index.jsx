...
const AnimalDetalhes = () => {
  ...
  useEffect(() => {
    document.title = "Detalhes do Animal - AutoFarm";
  }, []);
  ...
  useEffect(() => {
    getAll()
      .then((res) => {
        const historicosFiltrados = res.data.filter(
          (historico) => historico.animalId && historico.animalId._id === id
        );

        setHistoricos(historicosFiltrados);

        if (historicosFiltrados.length > 0) {
          const ultimaConsulta = historicosFiltrados.reduce(
            (maisRecente, atual) =>
              new Date(atual.data) > new Date(maisRecente.data)
                ? atual
                : maisRecente
          );
  ...
  return (
    <div className="w-full min-h-screen bg-emerald-50 ">
      <div className="pb-12">
        ...
        <div className="bg-white rounded-lg shadow-md p-8 w-full max-w-7xl mx-auto pb-36 justify-center">
            ...
            <div className="text-xl">
              ...
              <div className="space-y-4 mb-5">
                {historicos.map((historico) => (
                  ...
                  >
                    <h3>
                      <strong>Tratamento:</strong> {historico.tratamento}
                    </h3>
                    <p>
                      <strong>Data:</strong>{" "}
                    ...
                    <p>
                      <strong>Local:</strong> {historico.local}
                    </p>
                    <p>
                      <strong>Tamanho:</strong> {historico.tamanho} cm
                    </p>
                    <p>
                      <strong>Id:</strong> {historico.animalId?._id}
                    </p>
                  </div>
                ))}
                {historicos.length <= 0 && "Nenhum Hitórico"}
              </div>
              <button
      ...
      {modalAdd && <ModalAddHistorico closeModal={closeModal} animalId={id} />}
    </div>
  );
};
